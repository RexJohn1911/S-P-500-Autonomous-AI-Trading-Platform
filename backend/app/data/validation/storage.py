"""
Processed & Validated Market Data Storage Manager
Persists validated/cleaned market data into data/processed/ partitions, strictly preserving raw data immutability.
"""
...
class ProcessedDataStorage:
    """
    Manages validated and cleaned market data persistence under data/processed/{symbol}/{timeframe}/.
    Ensures raw data in data/raw/ is never overwritten or mutated.
    """
    ...
    def save_bars(self, bars: List[BarData], format: str = "parquet") -> Path:
        """Save cleaned and validated BarData records."""
        if not bars:
        ...
        if format == "json":
            ...
            logger.info(f"Saved {len(bars)} cleaned bars to {file_path}")
            return file_path
    ...
    def load_bars(self, symbol: str, timeframe: TimeFrame, format: str = "parquet") -> List[BarData]:
        """Load stored cleaned bars for a given symbol and timeframe."""
        target_dir = self._get_symbol_dir(symbol, timeframe)
